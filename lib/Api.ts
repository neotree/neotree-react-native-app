...
const Api = {
	webeditor: {
		async hospitals() {
			return GET<Hospital[]>('webeditor', '/api/hospitals', {
				defaultResponseData: [],
			});
		},

		async scripts(hospitalId: string) {
			return GET<ScriptWithItems[]>('webeditor', '/api/scripts/with-items', {
				defaultResponseData: [],
				data: {
					data: JSON.stringify({
		...
		async configKeys() {
			return GET<ConfigKey[]>('webeditor', '/api/config-keys', {
				defaultResponseData: [],
			});
		},

		async drugsLibrary() {
			return GET<DrugsLibraryItem[]>('webeditor', '/api/drugs-library', {
				defaultResponseData: [],
			});
		},

		async aliases() {
			return GET<Alias[]>('webeditor', '/api/aliases', {
				defaultResponseData: [],
			});
		},
	}
