...
import { createTablesIfNotExist, webSqlDbTransaction } from './db';
...
export async function syncData(opts?: { force?: boolean; }) {  
    ...
    console.log("--MY AUTH ===",networkState )

    if (authenticatedUser && networkState?.isConnected) {
    ...
    if (shouldSync) {
            ...
            await Promise.all(['scripts', 'screens', 'diagnoses', 'config_keys'].map(table => webSqlDbTransaction(
            ...
            configKeys.map((s: any) => {
                ...
                const values = ['?', '?', '?', '?', '?'].join(',');
                return webSqlDbTransaction(`insert or replace into config_keys (${columns}) values (${values});`, [
                    s.id,
                    s.config_key_id,
            ...
            scripts.map((s: any) => {
                ...
                const values = ['?', '?', '?', '?', '?', '?', '?'].join(',');
                promises.push(webSqlDbTransaction(`insert or replace into scripts (${columns}) values (${values});`, [
                    s.id,
                    s.script_id,
                    s.type || s.data.type,
                    JSON.stringify(s.data),
            ...
            screens.map((s: any) => {
                ...
                const values = ['?', '?', '?', '?', '?', '?', '?', '?'].join(',');
                promises.push(webSqlDbTransaction(`insert or replace into screens (${columns}) values (${values});`, [
                    s.id,
                    s.screen_id,
                    s.script_id,
                    s.position,
            ...
            diagnoses.map((s: any) => {
                ...
                const values = ['?', '?', '?', '?', '?', '?', '?', '?'].join(',');
                promises.push(webSqlDbTransaction(`insert or replace into diagnoses (${columns}) values (${values});`, [
                    s.id,
                    s.diagnosis_id,
                    s.script_id,
                    s.position,
