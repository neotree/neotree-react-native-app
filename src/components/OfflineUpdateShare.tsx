...
export function OfflineUpdateShare() {
  ...
  const confirmAndInstall = React.useCallback((imported: ImportedApk) => {
    ...
    Alert.alert('Update cannot be verified', message, [{ text: 'OK', style: 'cancel' }]);
  }, [doInstall]);
  ...
  const onImport = React.useCallback(async () => {
    ...
    try {
      ...
      Alert.alert('Could not open file', e?.message || 'Please choose the NeoTree update file (.apk).');
    } finally {
      setBusy(null);
    }
  }, [confirmAndInstall]);
  ...
  const onShare = React.useCallback(async () => {
    ...
    try {
      ...
      Alert.alert('Nothing to share yet', e?.message || 'This tablet does not have an update file to share yet.');
    } finally {
      setBusy(null);
    }
  }, []);
  ...
  return (
    <View style={styles.card}>
      <Text style={styles.title}>No internet? Share updates tablet to tablet</Text>
      <Text style={styles.subtitle}>
        Use this when the tablet has no internet. You can install an update from a file, or pass an
        update you already have to another tablet.
      </Text>

      <TouchableOpacity style={styles.action} onPress={onImport} disabled={busy !== null} activeOpacity={0.8}>
        <View style={styles.iconBox}>
          <Text style={styles.icon}>IN</Text>
        </View>
        <View style={styles.actionText}>
          <Text style={styles.actionTitle}>Install update from a file</Text>
          <Text style={styles.actionHint}>Choose an update someone sent you or copied onto this tablet.</Text>
        </View>
        {busy === 'import' ? <ActivityIndicator /> : <Text style={styles.chevron}>{'>'}</Text>}
      </TouchableOpacity>
      ...
      >
        <View style={styles.iconBox}>
          <Text style={styles.icon}>OUT</Text>
        </View>
        <View style={styles.actionText}>
          <Text style={styles.actionTitle}>Send update to another tablet</Text>
        ...
        {busy === 'share' ? <ActivityIndicator /> : <Text style={styles.chevron}>{'>'}</Text>}
      </TouchableOpacity>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  subtitle: { fontSize: 13, color: '#475569', marginTop: 4, marginBottom: 12, lineHeight: 19 },
  action: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 12,
  ...
  icon: { fontSize: 11, fontWeight: '700', color: '#25684A' },
  actionText: { flex: 1 },
  actionTitle: { fontSize: 15, fontWeight: '600', color: '#0F172A' },
  actionHint: { fontSize: 12, color: '#64748B', marginTop: 2, lineHeight: 17 },
