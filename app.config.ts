const NEOTREE_BUILD_TYPE = process.env.NEOTREE_BUILD_TYPE || 'development';

const isDev = NEOTREE_BUILD_TYPE === 'development';
const isDemo = NEOTREE_BUILD_TYPE === 'demo';
const isStage = NEOTREE_BUILD_TYPE === 'stage';
const isProd = NEOTREE_BUILD_TYPE === 'production';

export default ({ config }: any) => {
    const appConfig = getBuldConfig(config);
    return {
		...config,
        ...appConfig,
        extra: {
            ...config.extra,
            ...appConfig.extra,
            NEOTREE_BUILD_TYPE,
        },
	};
};

function getBuldConfig(config: any) {
	return {
		...(isDev ? {
			extra: {
				...config.extra,
				APP_ENV: 'LOCAL_DEV',
				eas: {
					...config?.extra?.eas,
					projectId: '88713878-bb93-4e2d-b54f-ed71db372a81',
				},
			},
		} : null),

		...(isDemo ? {
			// version: `${config.version}-DEMO`,
			name: `${config.name} (DEMO)`,
			slug: `${config.slug}-demo`,
			extra: {
				...config.extra,
				APP_ENV: 'DEMO',
				eas: {
					...config?.extra?.eas,
					projectId: '53b5b957-7cbe-4a1d-9bed-a4458a3baeb2',
				},
			},
		} : null),

		...(isStage ? {
			// version: `${config.version}-DEV`,
			name: `${config.name} (DEV)`,
			slug: `${config.slug}-dev`,
			extra: {
				...config.extra,
				APP_ENV: 'DEV',
				eas: {
					...config?.extra?.eas,
					projectId: '42a5fe96-9887-457c-91e7-9298cb4aa378',
				},
			},
		} : null),

		...(isProd ? {
			// version: `${config.version}-PROD`,
			extra: {
				...config.extra,
				APP_ENV: 'PROD',
				eas: {
					...config?.extra?.eas,
					projectId: '88713878-bb93-4e2d-b54f-ed71db372a81',
				},
			},
		} : null),
	};
}
